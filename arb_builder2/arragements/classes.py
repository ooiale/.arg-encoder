...
from typing import List, Optional, Tuple, Dict, TypeAlias, Union, Generic, TypeVar
...
T = TypeVar('T', bound=ArrangementType)

@dataclass
class Arrangements:
    # (preserves original order) - dont know if its needed
    arrangements: List[ArrangementType]
    # Grouped references for easy access
    _by_type: Dict[str, List[ArrangementType]] = field(default_factory=lambda: defaultdict(list), repr=False)
    
    def add(self, item: ArrangementType) -> None:
        """Add an item to the arrangements (called by parser)."""
        self.arrangements.append(item)
        type_name = type(item).__name__
        self._by_type[type_name].append(item)
    
    def __getattr__(self, name: str) -> List[ArrangementType]:
        """
        Direct access to lists by type name.
        Example: arrangements.MONS returns all MONS entries.
        """
        return self._by_type.get(name, [])
    
    def get_all(self) -> List[ArrangementType]:
        """Get all items in original order (for building)."""
        return self.arrangements
    
    def get_by_type(self, type_name: str) -> List[ArrangementType]:
        """Explicit method to get by type (alternative to __getattr__)."""
    ...
    # type-specific properties for IDE autocomplete
    @property
    def MONS(self) -> List[MONS]:
        return [item for item in self._by_type.get('MONS', []) if isinstance(item, MONS)]
    
    @property
    def TBOX(self) -> List[TBOX]:
        return [item for item in self._by_type.get('TBOX', []) if isinstance(item, TBOX)]

"""
  to edit later for rando stuff:
  all_mons = arr.MONS
