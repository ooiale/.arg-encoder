...
def parse_entry_by_type(entry_type: str, line: str, line_num: int) -> Any:
    ...
    if entry_type == "CHECKPT":
        ...
        range_val = parse_float(tokens[10])
        ...
        # Hardcoded exception for specific crystal entries
        if (name == "clystal" and node == "" and 
            abs(x - (-134.11)) < 0.01 and abs(y - (-88.51)) < 0.01 and abs(z - 34.34) < 0.01):
            spd = 1.0  # First crystal entry
        elif (name == "clystal" and node == "" and 
            abs(x - (-143.24)) < 0.01 and abs(y - (-139.16)) < 0.01 and abs(z - 22.36) < 0.01):
            spd = 1.0  # Second crystal entry
